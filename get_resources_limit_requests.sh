...
function get_resource_limits(){
    ...
    cpu_limits=$(oc get dc/$deployment_config -n $namespace -o jsonpath='{.spec.template.spec.containers[].resources.limits.cpu}')
    echo "export CPU_LIMITS=${cpu_limits}" > ./${namespace}/${deployment_config}/resource_vars
    
    memory_limits=$(oc get dc/$deployment_config -n $namespace -o jsonpath='{.spec.template.spec.containers[].resources.limits.memory}')
...
function get_resource_requests(){
    ...
    cpu_requests=$(oc get dc/$deployment_config -n $namespace -o jsonpath='{.spec.template.spec.containers[].resources.requests.cpu}')
    echo "export CPU_REQUESTS=${cpu_requests}" >> ./${namespace}/${deployment_config}/resource_vars
    
    memory_requests=$(oc get dc/$deployment_config -n $namespace -o jsonpath='{.spec.template.spec.containers[].resources.requests.memory}')
